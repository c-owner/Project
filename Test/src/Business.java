...
public class Business {
	...
	public void show(Seoul[] arr) {
		for(int a = 0; a < 4; a++) {
			System.out.println("--------");
			System.out.println(arr[a] + "");
			System.out.println("--------");
			System.out.println("배달담당\t배달시간\t배달량\t배달 요금");
			System.out.println("---------------------------");
			for (int b = 0; b < arr.length; b++ ) {
				if(arr[b] == null ) break;
				System.out.println(arr[b].name+"\t"+arr[b].time+"\t"+arr[b].cnt+"\t"+arr[b].pay);
			}
			System.out.println("---------------------------");
		}
	}
	public String AllShow(Seoul[] e) {
		String result ="";
		...
		for (int j = 0; j < e.length; j++ ) {
			if(e[j] == null ) break;
			result += e[j].name+"\t"+e[j].time+"\t"+e[j].cnt+"\t"+e[j].pay+"\n";
			}
		System.out.println("-------------------------");
		}
		return result;
